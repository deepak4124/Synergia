    ...
    for week in cal:
        ...
        for day in week:
            if day == 0:
                ...
                formatted_week.append((f'{day}', 'white', '#001F3F'))  # Highlight in yellow
            else:
                formatted_week.append(str(day))
        formatted_cal.append(formatted_week)

    layout = [
        [sg.Text(f'{calendar.month_name[month]} {year}', font=('Helvetica', 14))],
        [sg.Table(values=formatted_cal, headings=[day_abbr for day_abbr in calendar.day_abbr], auto_size_columns=False, justification='right', num_rows=6, col_widths=2)],
        [sg.Button('Close')]
    ]
...
def create_calendar_popup(current_day):
    ...
    return sg.Window('Calendar View', layout, keep_on_top=True, finalize=True)
'''def create_todo_popup(todo_list):
    layout = [
        [sg.Text('To-Do List', font=('Helvetica', 16))],
        [sg.Listbox(values=todo_list, size=(40, 10), key='-TODO_LIST-', enable_events=True)],
        [sg.InputText(key='-TODO_ITEM-', size=(30, 1)), sg.Button('Add', key='-ADD_TODO-')],
        [sg.Button('Remove', key='-REMOVE_TODO-')],
        [sg.Button('Close')]
    ]

    return sg.Window('To-Do List', layout, keep_on_top=True, finalize=True, return_keyboard_events=True)'''

'''return sg.Window('To-Do List', layout, keep_on_top=True, finalize=True)'''
'''def create_cli_popup():
    layout = [
    [sg.Text("Enter a shell command:")],
    [sg.InputText(key="-COMMAND-")],
    [sg.Button("Execute"), sg.Button("Clear Screen")],
    [sg.Text("Output:", pad=(0, 10))],
    [sg.Output(size=(80, 20), key="-OUTPUT-")]
]   
    return sg.Window('CLI', layout, background_color='lightgray',finalize=True)'''
